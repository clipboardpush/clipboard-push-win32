...
inline std::string Trim(const std::string& s) {
    ...
    auto end = s.end() - 1;
    while (end != start && std::isspace((unsigned char)*end)) {
        end--;
    }
    return std::string(start, end + 1);
}

inline std::string FormatDisplayName(const std::string& deviceName, const std::string& clientId) {
    std::string trimmed = Trim(deviceName);
    return trimmed.empty() ? clientId : trimmed;
}
